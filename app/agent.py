...
from app.types import DebateRound, DebateState, PillarScores
from app.utils import FilesystemJail, parse_node_input
...
async def evaluate_and_score_node(ctx: Context, node_input: dict) -> Event:
    ...
    if settings.mock_mode:
        # Fast local mock scoring for video recording without API rate limits or network latency
        sec_score = 0.80 if current_round == 1 else 0.89
        result_data = ScoreResult(
            scores=PillarScores(performance=0.88, scalability=0.90, security=sec_score, reliability=0.89, maintainability=0.91, cost_efficiency=0.86),
    ...
    # Yield intermediate state sync event without setting output to prevent ADK "Output already set" error
    yield Event(
